...
class BaseSensorViewClass(TemplateView):
    ...
    def pretty_trend(self, trend_decimal):
        """
        Human readable. Takes a float and rounds it up - people don't care
         about detail to that extent.
        """
    ...
    def get_trend_str(self, trend_duration, temperature_delta, humidity_delta):
        ...
        return "%s%s in the last %s" % (temperature_trend_str,
                                        temperature_units,
                                        self.pretty_duration(trend_duration)),\
               "%s%s in the last %s" % (humidity_trend_str,
                                        humidity_units,
                                        self.pretty_duration(trend_duration))
    ...
    def point_start_string(self):
        ...
        It's important to note that the month indexing starts at 0 so august
         is 7, instead of 8 (which is the python datetime repr)
        pointStart: Date.UTC(2012, 7, 14, 0, 0, 1),
        """
        ert = self.get_earliest_reading_time()
        return "Date.UTC(%s, %s, %s, %s, %s, %s)" % (ert.year,
                                                     ert.month - 1,
                                                     ert.day,
                                                     ert.hour,
                                                     ert.minute,
...
class LatestViewClass(BaseSensorViewClass):
    # Is this latest as in the most recent one that we have, no matter how
    #  old it is, or is it "current"
    def get_earliest_reading_time(self):
        # One hour ago
